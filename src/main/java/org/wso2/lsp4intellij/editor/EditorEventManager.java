...
import org.wso2.lsp4intellij.contributors.annotator.AnnotationInfo;
...
public class EditorEventManager {
     ...
     */
    public synchronized List<AnnotationInfo> getAnnotations() {
        this.codeActionSyncRequired = false;
        return this.annotations;
    }

    public synchronized void setAnnotations(List<AnnotationInfo> annotations) {
     ...
     */
    public void reformat(ReformatAccept accept) {
        pool(() -> {
            if (editor.isDisposed()) {
                return;
            }
...
//                    invokeLater(() -> applyEdit(toEither((List<TextEdit>) formatting), "Reformat document", false));
     ...
     */
    public void reformatSelection(ReformatAccept accept) {
        pool(() -> {
            if (editor.isDisposed()) {
                return;
            }
            ...
            request.thenAccept(formatting -> {
                ...
                accept.Accept(calculateTextEdit(toEither((List<TextEdit>) formatting)));
            });
        });
    }
    ...
    public String generateDoc(int offset) {
        ...
        try {
            ...
            return HoverHandler.getHoverString(hover);
        } catch (TimeoutException e) {
            LOG.warn(e);
    ...
    public LookupElement createLookupItem(CompletionItem item) {
        ...
        InsertReplaceEdit insertReplaceEdit = null;
        Either<TextEdit, InsertReplaceEdit> textEditOrInsert = item.getTextEdit();
        if(textEditOrInsert != null){
            textEdit = textEditOrInsert.getLeft();
            insertReplaceEdit = textEditOrInsert.getRight();
        }
    ...
    public String calculateTextEdit(List<Either<TextEdit, InsertReplaceEdit>> edits) {
        if (edits == null) {
            LOG.warn("Received edits list is null.");
            return null;
        }
        ...
        StringBuilder document = new StringBuilder(editor.getDocument().getText());
            ...
            codeActionResp.forEach(element -> {
                ...
                if (element.isLeft()) {
                    ...
                    if ((diagnosticContext == null || diagnosticContext.isEmpty()) && anonHolder != null && !codeActionSyncRequired) {
                        ...
                        SmartList<AnnotationInfo> asList = (SmartList<AnnotationInfo>) this.anonHolder;
